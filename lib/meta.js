...
// Sayfa feed'indeki son yorumlari topla (otomatik tarama icin)
export async function listPageComments(pageId, token) {
  const feed = await graphGet(
    `${pageId}/feed`,
    { fields: 'id,comments.limit(25){id,message,from,created_time}', limit: '10' },
    token
  )
  const out = []
  for (const post of feed.data || []) {
    for (const c of post.comments?.data || []) {
      out.push({ id: c.id, message: c.message || '', from: c.from || {}, postId: post.id })
    }
  }
  return out
}
