...
function ffmpegDrawColor(c) {
  const m = String(c || '#ffffff').trim().match(/^#?([0-9A-Fa-f]{6})$/)
  return m ? ('0x' + m[1]) : 'white'
}

// Videoya reklam/sahne yazisi bindir (belirli saniye araligi veya tum video)
export async function overlayText({ inputPath, outPath, text, start = 0, end = null, position = 'bottom', color = 'white', fontSize }) {
  if (!fs.existsSync(inputPath)) throw new Error('video dosyasi bulunamadi')
  const raw = String(text || '').replace(/\r\n/g, '\n').trim().slice(0, 240)
  if (!raw) throw new Error('yazi bos olamaz')
  const info = await probeVideoInfo(inputPath)
  const font = findDrawtextFont()
  if (!font) throw new Error('Yazi fontu yok (DejaVu/Arial)')
  const fsSize = Math.max(24, Math.min(96, Number(fontSize) || Math.round(info.height * 0.05)))
  const tmp = path.join(os.tmpdir(), `vtxt_${Date.now()}.txt`)
  fs.writeFileSync(tmp, raw, 'utf8')
  const y = position === 'top' ? 'h*0.07' : position === 'center' ? '(h-text_h)/2' : 'h-text_h-(h*0.08)'
  const s = Math.max(0, Number(start) || 0)
  const e = end == null || end === '' ? info.duration : Number(end)
  const timed = Number.isFinite(e) && e > s
  const enable = timed ? `:enable='between(t,${s},${e})'` : ''
  const fontcolor = ffmpegDrawColor(color)
  const vf = `drawtext=fontfile='${ffmpegFilterPath(font)}':textfile='${ffmpegFilterPath(tmp)}':reload=0:fontsize=${fsSize}:fontcolor=${fontcolor}:borderw=3:bordercolor=black@0.85:box=1:boxcolor=black@0.42:boxborderw=14:line_spacing=10:x=(w-text_w)/2:y=${y}${enable}`
  const args = ['-y', '-i', inputPath, '-vf', vf, '-c:v', 'libx264', '-preset', 'veryfast', '-pix_fmt', 'yuv420p']
...
// Videoya logo/kampanya gorseli bindir (konum yuzde, boyut, sure araligi)
export async function overlayImage({ inputPath, imagePath, outPath, start = 0, end = null, xPct = 50, yPct = 80, sizePct = 28 }) {
  if (!fs.existsSync(inputPath)) throw new Error('video dosyasi bulunamadi')
  if (!fs.existsSync(imagePath)) throw new Error('gorsel bulunamadi')
  const info = await probeVideoInfo(inputPath)
  const size = Math.max(8, Math.min(80, Number(sizePct) || 28))
  const xf = Math.max(0, Math.min(100, Number(xPct) || 50)) / 100
  const yf = Math.max(0, Math.min(100, Number(yPct) || 80)) / 100
  let tw = Math.round(info.width * size / 100)
  if (tw % 2) tw -= 1
  tw = Math.max(16, tw)
  const s = Math.max(0, Number(start) || 0)
  const e = end == null || end === '' ? info.duration : Number(end)
  const timed = Number.isFinite(e) && e > s
  const enable = timed ? `:enable='between(t,${s},${e})'` : ''
  const x = `max(0\\,min(W-w\\,W*${xf}-w/2))`
  const y = `max(0\\,min(H-h\\,H*${yf}-h/2))`
  const filter = `[1:v]format=rgba,scale=${tw}:-2[ov];[0:v][ov]overlay=x='${x}':y='${y}'${enable}`
  const args = [
    '-y', '-i', inputPath, '-loop', '1', '-i', imagePath,
    '-filter_complex', filter,
    '-c:v', 'libx264', '-preset', 'veryfast', '-pix_fmt', 'yuv420p',
  ]
  if (info.hasAudio) args.push('-c:a', 'aac', '-b:a', '128k')
  else args.push('-an')
  if (info.duration > 0) args.push('-t', String(info.duration))
  args.push('-movflags', '+faststart', outPath)
  await runFfmpeg(args, outPath)
  return outPath
}
