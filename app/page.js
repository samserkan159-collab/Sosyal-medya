...
import VideoTranslator from '@/components/studio/VideoTranslator'
import AiVideoStudio from '@/components/studio/AiVideoStudio'
import {
...
import {
  ...
  Upload, Wand2, Phone, Info, TrendingUp, AlertTriangle, Radio, Menu, CalendarClock, Trash2, Film, Languages, Clapperboard, LogOut,
} from 'lucide-react'
...
const NAV = [
  ...
  { id: 'translate', label: 'Video Cevirici', icon: Languages },
  { id: 'kling', label: 'AI Video', icon: Clapperboard },
  ...
  return (
    <>
      <div className="flex items-center gap-2.5 px-5 py-5">
        ...
        <div>
          <p className="text-sm font-semibold leading-none text-zinc-100">ASM Cockpit</p>
          <p className="mt-1 text-[11px] text-zinc-500">ASM</p>
        </div>
      </div>
      ...
      <main className="md:ml-64">
        <header className="sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-zinc-800 bg-zinc-950/80 px-4 py-4 backdrop-blur sm:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <LogOut className="h-3.5 w-3.5 sm:mr-1.5" /> <span className="hidden sm:inline">Cikis Yap</span>
            </Button>
          </div>
        </header>
        ...
        <div className="p-4 sm:p-8">
          ...
          {tab === 'translate' && <VideoTranslator />}
          {tab === 'kling' && <AiVideoStudio />}
...
function SettingsModule({ page, config, reload }) {
  ...
  useEffect(() => { if (page) setF({ commentTemplate: page.commentTemplate || '', dmTemplate: page.dmTemplate || '', ytReplyTemplate: page.ytReplyTemplate || '', whatsappNumber: page.whatsappNumber || '', autoReplyActive: page.autoReplyActive !== false }) }, [page])
        ...
        <Card className="border-zinc-800 bg-zinc-900/70">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-950/50 p-4">
              <div><p className="text-sm font-medium">Otomatik Yanit Motoru</p><p className="text-xs text-zinc-500">Facebook + YouTube fiyat yorumlarina otomatik cevap</p></div>
              <Switch checked={f.autoReplyActive} onCheckedChange={(v) => setF({ ...f, autoReplyActive: v })} />
            </div>
            <div><label className="mb-1.5 block text-xs text-zinc-500">Facebook — videoya gelen yoruma cevap</label><Textarea rows={3} value={f.commentTemplate} onChange={(e) => setF({ ...f, commentTemplate: e.target.value })} className="border-zinc-800 bg-zinc-950" /></div>
            <div><label className="mb-1.5 block text-xs text-zinc-500">Messenger — ozel cevap (sadece yorum yazana)</label><Textarea rows={3} value={f.dmTemplate} onChange={(e) => setF({ ...f, dmTemplate: e.target.value })} className="border-zinc-800 bg-zinc-950" /></div>
            <div><label className="mb-1.5 block text-xs text-zinc-500">YouTube — videoya gelen yoruma cevap</label><Textarea rows={3} value={f.ytReplyTemplate} onChange={(e) => setF({ ...f, ytReplyTemplate: e.target.value })} className="border-zinc-800 bg-zinc-950" /></div>
      ...
      <Card className="border-zinc-800 bg-zinc-900/70">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-lg border border-amber-500/20 bg-amber-950/10 p-3 text-xs text-amber-300/80">
            <b>Not:</b> API anahtarlari guvenlik nedeniyle sunucu tarafinda <code className="text-amber-200">.env</code> dosyasinda tutulur.
