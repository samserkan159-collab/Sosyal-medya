...
| **🤖 Telegram Bot** | Webhook üzerinden bildirim ve ses→metin (Whisper) girişi |
...
- **AI:** `emergentintegrations` (GPT-4o metin, GPT-4o Vision görsel analiz, Whisper ses→metin)
- **Backend mimari:** Tek catch-all route (`/app/api/[[...path]]/route.js`) + `lib/` altında servis modülleri
- **Süreç yönetimi:** Supervisor (Next.js `0.0.0.0:3000`)
...
Gereksinimler: Node.js, Yarn, MongoDB, FFmpeg.

```bash
# 1) Bağımlılıklar
yarn install

# 2) FFmpeg (video render/kesme için zorunlu)
sudo apt-get update && sudo apt-get install -y ffmpeg

# 3) .env dosyasını doldurun (aşağıya bakın)

# 4) Servisi başlat (supervisor ile)
sudo supervisorctl restart nextjs
```

Uygulama `0.0.0.0:3000` üzerinde çalışır; tüm backend istekleri `/api` ön eki ile yönlendirilir.

---

## 🔑 Ortam Değişkenleri (.env) — Nasıl Bağlanılır?

> AI özellikleri `EMERGENT_LLM_KEY` ile **anında** çalışır. Diğer entegrasyonlar için ilgili anahtarları girmeniz yeterlidir; girmezseniz o özellik "anahtar gerekli" uyarısı verir.

### Zorunlu (sistem tarafından yönetilir — DEĞİŞTİRMEYİN)
| Değişken | Açıklama |
|----------|----------|
| `MONGO_URL` | MongoDB bağlantısı |
| `DB_NAME` | Veritabanı adı |
| `NEXT_PUBLIC_BASE_URL` | Uygulamanın dış URL'si |
| `CORS_ORIGINS` | İzinli origin'ler |

### AI (hazır)
| Değişken | Nereden? |
|----------|----------|
| `EMERGENT_LLM_KEY` | Emergent evrensel anahtarı (GPT-4o, Vision, Whisper) |
| `AI_PROVIDER`, `AI_MODEL` | Sağlayıcı/model (varsayılan gpt-4o) |
...
|--------|----------|-----------|
| **OpenAI GPT-4o / Vision / Whisper** | İçerik üretimi, görsel OCR, ses→metin | Emergent LLM Key (hazır) |
