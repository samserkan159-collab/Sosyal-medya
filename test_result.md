...
agent_communication:
    -agent: "main"
    -message: "FIX + enhancement to verify. (1) BUGFIX single-scene render performance: root cause was zoompan being fed by an infinite -loop input (frame multiplication). Fixed lib/reels.js to feed a single image frame. VERIFY: POST /api/studio/render {posterDataUrl:'data:image/png;base64,<small PNG>', audioMode:'silent'} -> poll GET /api/studio/render/<jobId>; must reach DONE in UNDER ~20s (previously 5+ min) with a valid non-empty video/mp4 at videoUrl. Also verify preset audio single-scene DONE fast, and a 2-scene render still DONE fast with valid MP4. (2) AI context: POST /api/studio/suggest-labels {image:'data:image/png;base64,<PNG>', context:'oto klima gazi dolum cihazi'} -> 200 JSON with title/badges/features/cta; content should reflect the given context. Report timings and whether MP4s are produced."
    -agent: "testing"
